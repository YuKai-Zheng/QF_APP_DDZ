...
# ADD-BEGIN by dantezhu in 2016-01-28 14:37:38
# 为了解决有些不可见字符的问题
        if options.fileofencryptkey:
            with open(options.fileofencryptkey, 'rb') as f:
                self._encryptkey = f.read()
        if options.prefixencryptkey:
            self._encryptkey = options.prefixencryptkey + self._encryptkey
# ADD-END
    ...
    # by dantezhu
    def get_output_file_path(self, luafile, suffix):
        """
        ...
        luac_filepath = ""
        # Unknow to remove 'c' 
        relative_path = self.get_relative_path(luafile) + suffix
        ...
        for src_dir in self._src_dir_arr:
            for lua_file in self._lua_files[src_dir]:
                ...
                if self._isEncrypt == True:
                    ...
                    dst_lua_file = self.get_output_file_path(lua_file, "c")
    ...
    def parse_args(self, argv):
        ...
        parser.add_option("-b", "--encryptsign",
                          ...
                          help="encrypt sign")
        parser.add_option("-f", "--fileofencryptkey",
                          dest="fileofencryptkey",default="",
                          help="file of encryptkey")
        parser.add_option("-p", "--prefixencryptkey",
                          dest="prefixencryptkey",default="",
                          help="prefix of encryptkey")
